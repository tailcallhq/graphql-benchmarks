...
const buildResultsTable = (whichBench: number): string => {
  ...
  const tableData = [["Server", "Requests/sec", "Latency (ms)", "Relative"]];
...
const analyzeBenchmarks = (resultFiles: string[]) => {
  ...
  for (let index = 0; index < 3; index++) {
    const whichBench = index + 1;

    const resultFilesForBench = resultFiles.filter((file) =>
      file && file.startsWith(`bench${whichBench}`)
    );

    processResultFiles(resultFilesForBench);
